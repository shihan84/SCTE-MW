...
function startRTMPServer() {
    console.log('🎥 Starting Simple RTMP Server on port', CONFIG.rtmpPort);
    
    // Create a simple TCP server to handle RTMP connections
    rtmpServer = net.createServer((socket) => {
        console.log('📺 RTMP Client connected from:', socket.remoteAddress);
        ...
        socket.on('data', (data) => {
            ...
            // Look for stream path in RTMP data
            const streamMatch = dataStr.match(/live\/(\w+)/);
            if (streamMatch && !streamId) {
                streamId = streamMatch[1];
                console.log(`✅ Detected Stream: ${streamId}`);
                
                // Start FFmpeg processing after a short delay
                setTimeout(() => {
        ...
        socket.on('error', (err) => {
            console.error('RTMP Socket Error:', err.message);
        });
    });
    
    rtmpServer.listen(CONFIG.rtmpPort, () => {
        console.log('📡 Simple RTMP Server listening on port', CONFIG.rtmpPort);
        console.log('📺 OBS URL: rtmp://localhost:' + CONFIG.rtmpPort + '/live/stream1');
    });
    
    rtmpServer.on('error', (err) => {
        console.error('RTMP Server Error:', err.message);
        if (err.code === 'EADDRINUSE') {
            console.log('Port 1935 is already in use. Trying alternative approach...');
            startFFmpegRTMPServer();
        }
    });
}

// Alternative: Use FFmpeg as RTMP server
function startFFmpegRTMPServer() {
    console.log('🎥 Starting FFmpeg RTMP Server on port', CONFIG.rtmpPort);
    
    // Use FFmpeg to create an RTMP server
    const ffmpegRTMPArgs = [
        '-f', 'flv',
        '-listen', '1',
        '-i', `rtmp://localhost:${CONFIG.rtmpPort}/live/stream1`,
        '-c:v', 'copy',
        '-c:a', 'copy',
        '-f', 'mpegts',
        ...
        `srt://${CONFIG.flussonicHost}:${CONFIG.flussonicPort}?streamid=stream1&pkt_size=${CONFIG.srtPacketSize}`
    ];
    
    const rtmpProcess = spawn('ffmpeg', ffmpegRTMPArgs);
    
    rtmpProcess.stdout.on('data', (data) => {
        console.log('FFmpeg RTMP Server:', data.toString());
    });
    
    rtmpProcess.stderr.on('data', (data) => {
        const output = data.toString();
        console.log('FFmpeg RTMP Server:', output);
        
        if (output.includes('Server started')) {
            console.log('✅ FFmpeg RTMP Server started successfully');
    ...
    rtmpProcess.on('close', (code) => {
        console.log('FFmpeg RTMP Server exited with code', code);
    });
    
    rtmpProcess.on('error', (error) => {
        console.error('FFmpeg RTMP Server Error:', error.message);
    });
    
    activeFFmpegProcesses['rtmp-server'] = rtmpProcess;
}
...
function startServers() {
    ...
    app.listen(CONFIG.apiPort, () => {
        ...
        console.log('📡 RTMP Dashboard: http://localhost:' + CONFIG.apiPort + '/rtmp-dashboard.html');
